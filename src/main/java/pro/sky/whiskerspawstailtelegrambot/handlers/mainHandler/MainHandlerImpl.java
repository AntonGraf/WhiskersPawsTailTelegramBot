...
public class MainHandlerImpl implements MainHandler {
  ...
  public MainHandlerImpl(StandardReplyHandler standardReplyHandler,
      CallbackQueryHandler callbackQueryHandler, StateHandler stateHandler,
...
//    StateAdoptiveParent stateAdoptiveParent = checkingState(baseInfo);
...
//    }
  ...
  private SendMessage processingIncomingMessages(GetBaseInfoFromUpdate baseInfo) {
    ...
    try {
      sendMessage = stateChangingButtons.handleClick(textMessage);
      if (sendMessage != null) {
        return sendMessage;
      }
      ...
      if (baseInfo.isCallbackQuery()) {
        ...
        Message message = baseInfo.getMessage();
